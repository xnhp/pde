...
class OsgiManifestCompletionContributor : CompletionContributor() {
  init {
    ...
    extend(
      CompletionType.BASIC, header(EXPORT_PACKAGE), HeaderParametersProvider(VERSION_ATTRIBUTE, "$USES_DIRECTIVE:")
    )
    extend(
      CompletionType.BASIC,
    ...
    extend(
      CompletionType.BASIC, directive(VISIBILITY_DIRECTIVE), ValueProvider(VISIBILITY_PRIVATE, VISIBILITY_REEXPORT)
    )
    extend(
      CompletionType.BASIC, directive(RESOLUTION_DIRECTIVE), ValueProvider(RESOLUTION_MANDATORY, RESOLUTION_OPTIONAL)
    )
  }
}
