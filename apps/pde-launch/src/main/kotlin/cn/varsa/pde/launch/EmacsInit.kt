...
object EmacsInit {
  ...
  fun main(args: Array<String>): Int {
    ...
    ensureDirLocals(workspaceRoot, configPath, resourceFilters)
  ...
  private fun buildResourceFilters(
    context: LaunchConfigContext,
    workspaceRoot: Path,
    workspaceEntries: List<WorkspaceBundleDescriptor>
  ): List<String> {
    val allowedProjects = workspaceEntries.mapNotNull { entry ->
      val moduleDir = entry.path.toAbsolutePath().normalize()
      if (!moduleDir.startsWith(workspaceRoot)) return@mapNotNull null
      moduleDir
    }.toSet()
    val repoRoots = context.config.bundlesPerRepo
      .mapNotNull { resolveRepoRoot(context.baseDir, it.repo) }
      .distinct()
    if (repoRoots.isEmpty()) return emptyList()
    val filters = mutableSetOf<String>()
    repoRoots.forEach { repoRoot ->
      if (!Files.isDirectory(repoRoot)) return@forEach
  ...
  private fun resolveRepoRoot(baseDir: Path, raw: String): Path? {
    val path = Paths.get(raw)
    val candidates = if (path.isAbsolute) {
      listOf(path)
    } else {
      listOfNotNull(baseDir.resolve(path), baseDir.parent?.resolve(path))
    }
    return candidates
  ...
  private fun ensureDirLocals(workspaceRoot: Path, configPath: Path, resourceFilters: List<String>) {
    ...
    """.trimIndent()
    val filtersLiteral = if (resourceFilters.isEmpty()) {
      null
    } else {
      resourceFilters.joinToString(" ", "(", ")") { "\"$it\"" }
    }
    val resourceFiltersEntry = filtersLiteral?.let {
      "(eglot-workspace-configuration . (:java (:project (:resourceFilters $it))))"
    } ?: ""
    val content = """
      ;; Generated by pde emacs-init. Safe to delete and re-run.
      ((nil . ((bookmark-default-file . ".emacs.d/bookmarks-project")
        $resourceFiltersEntry
        (eval . (progn
          (require 'project)
          (require 'cl-lib)
