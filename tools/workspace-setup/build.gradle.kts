...
val appBsn = "cn.varsa.pde.workspace_setup"
...
val appBundleJar by tasks.registering(Jar::class) {
  description = "Package the workspace setup as an OSGi bundle"
  group = "build"
  ...
  manifest {
    attributes(
      ...
      "Bundle-Name" to "pde Workspace Setup",
      ...
      "Require-Bundle" to listOf(
        ...
        "org.eclipse.jdt.core", "org.eclipse.jdt.launching"
      ).joinToString(",")
    )
  }
...
val materializeWorkspaceSetupRuntime = registerMaterializeRuntime(
  taskName = "materializeWorkspaceSetupRuntime",
  launcherJar = launcherJar,
...
materializeWorkspaceSetupRuntime {
...
val workspaceSetupRuntimeZip by tasks.registering(Zip::class) {
  description = "Assemble the workspace setup runtime zip"
  group = "build"
  dependsOn(materializeWorkspaceSetupRuntime)
  from(runtimeDir)
  archiveFileName = "workspace-setup-runtime.zip"
  destinationDirectory = layout.buildDirectory.dir("libs")
}

tasks.named("assemble") {
  dependsOn(workspaceSetupRuntimeZip)
}
