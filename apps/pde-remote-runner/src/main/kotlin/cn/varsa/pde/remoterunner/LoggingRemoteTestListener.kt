...
class LoggingRemoteTestListener(
  ...
  override fun runStarted(event: RunStartedEvent) {
    ...
    out.println("[INFO] Remote test session started (tests=$countText, protocol=$versionText)")
  }

  override fun testStarted(descriptor: RemoteTestDescriptor) {
    if (shouldLog(descriptor.printableName)) {
      out.println("[TEST] START ${descriptor.printableName}")
    }
  }
  ...
  override fun testFinished(result: RemoteTestResult) {
    ...
    val prefix = when (result.status) {
      RemoteTestStatus.PASSED -> "[PASS]"
      RemoteTestStatus.FAILED -> "[FAIL]"
      RemoteTestStatus.ERROR -> "[ERROR]"
    }
  ...
  override fun runEnded(summary: RemoteTestSummary) {
    ...
    out.println("[INFO] Remote test session finished at ${summary.endedAt?.atZone(ZoneOffset.UTC)?.let(isoFormatter::format) ?: "unknown"}")
    out.println("[INFO] Tests=${summary.finishedTests} failed=${summary.failures} errors=${summary.errors} elapsedMs=${elapsed ?: -1}")
  }

  override fun runStopped(summary: RemoteTestSummary) {
    out.println("[WARN] Remote test session stopped before completion")
    runEnded(summary)
  }
