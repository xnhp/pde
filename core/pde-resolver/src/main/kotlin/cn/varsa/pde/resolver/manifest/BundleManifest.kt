...
import cn.varsa.pde.resolver.support.*
...
/**
 * Minimal tokenizer for OSGi-style parameter lists.
 */
class QuotedTokenizer(private val value: String, private val delim: String) {
  private var i = 0
  var separator: Char = ','
    private set

  fun nextToken(extraDelim: String = delim): String? {
    ...
    while (i < value.length) {
      val c = value[i]
      if (!quoted && (extraDelim.indexOf(c) >= 0)) {
        separator = c
        i++
        break
      }
