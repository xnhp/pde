  ...
  internal fun getPackageReferences(element: PsiElement): Array<PsiReference> {
    ...
    return object : PackageReferenceSet(packageName, element, offset) {
      override fun resolvePackageName(
        ...
        return context?.subPackages?.filter { unwrappedPackageName == it.name }?.toMutableList() ?: mutableListOf()
      }
...
object BooleanValueParser : StandardHeaderParser() {
  override fun annotate(header: Header, holder: AnnotationHolder): Boolean {
    ...
    if (headerValue is HeaderValuePart && headerValue.unwrappedText.toBooleanStrictOrNull() == null) {
      holder.createError(
        message("manifest.lang.shouldBe", ECLIPSE_EXTENSIBLE_API, "true or false"), headerValue.highlightingRange
      )
      return true
    }
    return false
      ...
      clause.getDirectives().run {
        ...
        firstOrNull { it.name == SINGLETON_DIRECTIVE }?.getValueElement()
          ?.takeUnless { arrayOf("true", "false").contains(it.unwrappedText) }?.let {
            holder.createError(
              message("manifest.lang.shouldBe", SINGLETON_DIRECTIVE, "true or false"), it.highlightingRange
            )
            annotated = true
          }
      }
  ...
  override fun annotate(header: Header, holder: AnnotationHolder): Boolean {
    ...
    header.headerValues.mapNotNull { it as? Clause }.forEach { clause ->
      ...
      if (clause.getDirectives().any { it.name != INCLUDE_DIRECTIVE && it.name != EXCLUDE_DIRECTIVE }) {
        holder.createError(
          message("manifest.lang.specifyDirectivesOnly", "$INCLUDE_DIRECTIVE / $EXCLUDE_DIRECTIVE"), clause.textRange
        )
        annotated = true
      }
    }
  ...
  override fun annotate(header: Header, holder: AnnotationHolder): Boolean {
    ...
    if (clauses.size > 1) {
      ...
      val range = try {
        ...
        holder.createError(
          message("manifest.lang.invalidRange", e.message ?: "Unknown"), versionAttr?.textRange ?: clause.textRange
        )
        annotated = true
        VersionRangeAny
      }
