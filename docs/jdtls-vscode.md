...
3. Run `pde lsp init` from the issue root (or `--issue-dir`/`--config` as usual).

This now does two things relevant to VS Code, in addition to the usual
`.project`/`.classpath` generation:

- Writes `.vscode/settings.json` (only if it doesn't already exist — never clobbers
...
- Writes `.projectile` (same as the Eglot flow), harmless for VS Code.
...
- Re-run `pde lsp init` whenever workspace bundles change; VS Code's JDT LS has the
